...
while(1){
    ...
    // Attempt to send the first reply to the client
    ssize_t valsent;
    int retries = 0;
    bool message_sent = false;
    while (!message_sent && retries < MAX_RETRIES) {
        valsent = sendto(server_fd, CHAT_OK_REPLY, strlen(CHAT_OK_REPLY), 0,
        ...
        // Set up timeout for receiving the next message
        fd_set readfds;
        FD_ZERO(&readfds);
        ...
        if (select_result == -1) {
            ...
            // Timeout occurred, increment retries and try again
            retries++;
        } else {
            // Data is available to be read, break out of the retry loop
            message_sent = true;
            break;
        }
    }
    ...
    if (!message_sent) {
        printf("Max retries reached. Unable to send message to client.\n");
        close(server_fd);
        exit(EXIT_FAILURE);
    }
    ...
    // Send "HELLO_SSL_SERVER" message to client with retry mechanism and timeout
ssize_t valuesent;
int retries_hello_ssl_server = 0;
bool message_sent_hello_ssl_server = false;
while (!message_sent_hello_ssl_server && retries_hello_ssl_server < MAX_RETRIES) {
    valuesent = sendto(server_fd, HELLO_SSL_SERVER, strlen(HELLO_SSL_SERVER), 0,
                        (const struct sockaddr *)&client_addr, sizeof(client_addr));
    if (valuesent == -1) {
        perror("sendto");
        close(server_fd);
    ...
    // Set up timeout for receiving the acknowledgment
    fd_set readfds;
    FD_ZERO(&readfds);
    FD_SET(server_fd, &readfds);
    ...
    if (select_result == -1) {
        ...
        // Timeout occurred, increment retries and try again
        retries_hello_ssl_server++;
    } else {
        // Data is available to be read, break out of the retry loop
        message_sent_hello_ssl_server = true;
        break;
    }
}

if (!message_sent_hello_ssl_server) {
    printf("Max retries reached. Unable to send 'HELLO_SSL_SERVER' message to client.\n");
    close(server_fd);
    exit(EXIT_FAILURE);
}
...
void handle_client_mode(const std::string& server_hostname) {
    ...
    // Send "chat_hello" message to server with retry mechanism
    ssize_t val_sent;
    int retries = 0;
    bool message_sent = false;
    while (!message_sent && retries < MAX_RETRIES) {
        val_sent = sendto(client_socket, CHAT_HELLO, strlen(CHAT_HELLO), 0,
    ...
    if (!message_sent) {
        printf("Max retries reached. Unable to send message to server.\n");
        close(client_socket);
        exit(EXIT_FAILURE);
    }
    ...
    // Send "start_ssl" message to server with retry mechanism
    ssize_t value_sent;
    int retries_start_ssl = 0;
    bool message_sent_start_ssl = false;
    while (!message_sent_start_ssl && retries_start_ssl < MAX_RETRIES) {
        value_sent = sendto(client_socket, start_ssl, strlen(start_ssl), 0,
                            (const struct sockaddr *)&address, sizeof(address));
        if (value_sent == -1) {
            perror("sendto");
            close(client_socket);
        ...
        if (select_result == -1) {
            ...
            // Data is available to be read, break out of the retry loop
            message_sent_start_ssl = true;
            break;
        }
    }

    if (!message_sent_start_ssl) {
        printf("Max retries reached. Unable to send 'start_ssl' message to server.\n");
        close(client_socket);
        exit(EXIT_FAILURE);
    }
